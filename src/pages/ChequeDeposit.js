...
export default function ChequeDeposit() {
    ...
    const getData = async () => {// function getData to get terminal info when pass the terminalNumber from the url of the current page 
        const { data } = await axios.get(`http://localhost:8080/api/findTransaction/transactions/chq/id?id=${atmid.pathname.split('/')[3].split(',')[1]}`, {
            headers: {
...
console.log(data);
    ...
    return (
        ...
        <>
            <Helmet>
                <title> Cheque Deposit Transaction Detailes</title>
            </Helmet>
            ...
            Cheque Deposit Transaction Detailes
                                ...
                                <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)' }}>
                                    <>
                                        ...
                                        <FormLabel style={{ margin: "10px", width: "200px", marginTop: "10px" }} id="id">{dayjs(data?.CHD_DATE).format("MM/DD/YYYY")}</FormLabel>
                                    
                                    </>

                                    <>
                                        <FormLabel style={{ margin: "15px", width: "200px", marginTop: "10px", color: 'green' }} id="id">Card</FormLabel>
                                        <FormLabel style={{ margin: "10px", width: "200px", marginTop: "10px" }} id="id">{data?.CHD_CARD}</FormLabel>
                                        <FormLabel style={{ margin: "15px", width: "200px", marginTop: "10px", color: 'green' }} id="id">Transaction Num</FormLabel>
                                        <FormLabel style={{ margin: "10px", width: "200px", marginTop: "10px" }} id="id">{data?.CHD_TRANSACTION}</FormLabel>
                                        <FormLabel style={{ margin: "15px", width: "200px", marginTop: "10px", color: 'green' }} id="id">Response Code</FormLabel>
                                        <FormLabel style={{ margin: "10px", width: "200px", marginTop: "10px" }} id="id">{data?.CHD_RESPONSECODE}</FormLabel>
